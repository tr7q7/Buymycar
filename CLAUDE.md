...
| 2026-07-07 | Feature — clic point graphique → annonce | Clic sur un point du scatter défile vers la ligne/carte de l'annonce (data-listing-id, scroll vers l'élément visible) + surbrillance 2.6s |
| 2026-07-07 | Fix build Vercel — pnpm | package.json : pnpm.overrides (hono) retiré + packageManager pnpm@11.9.0 ; pnpm-workspace.yaml committé (allowBuilds msw/sharp false, verifyDepsBeforeRun false) |
| 2026-07-07 | Paiement MVP — crédits + Stripe (étapes 1→5) | DB SQLAlchemy (customers/payments/searches, DATABASE_URL SQLite↔Postgres) ; /credits/init & /credits ; /search exige email + décrément atomique (402 NEED_CREDITS) ; Stripe Checkout prix dynamique 200/eur (pas d'abonnement) + webhook idempotent (PK payments) ; front : email persisté localStorage, affichage crédits, bouton achat, retour ?payment=success ; pas de login. 92 tests verts |
| 2026-07-07 | Audit paiement + docs Go Live | Audit : code OK (endpoints, décrément atomique, idempotence, aucune clé secrète front, CORS Vercel OK) MAIS code paiement PAS déployé (API en ligne = version pré-paiement, /credits & /stripe/webhook en 404) ; README + DEPLOY.md + GO_LIVE.md (checklist push+redeploy, tests carte 4242, risques, probabilité) |
| 2026-07-07 | Anti-abus crédits + compteur + UX | Table visitors + init_free_credits (2 gratuits max par email ET par visitor_id ; device_blocked) ; /credits/init & /search reçoivent visitor_id ; front : store crédits en contexte (CreditsProvider), compteur vert global en haut (init/recherche/achat/retour Stripe), message « déjà utilisés sur cet appareil », bouton « Nouvelle analyse » primaire en haut+bas du dashboard ; GO_LIVE : checklist Stripe Live (clés test→live sur Render, code inchangé). 98 tests verts, E2E navigateur OK |
